...
Aulas organizadas cronologicamente (`Aula01` a `Aula27`), cobrindo revisão, funções e estruturas de dados sequenciais, conjuntos, dicionários, recursividade e memoização, bibliotecas/módulos, corretude de algoritmos, tratamento de exceções, TDD, análise de eficiência, algoritmos de ordenação (bogo/bubble, selection/insertion, heap, merge, quick sort), algoritmos de busca (linear, binária, quickselect) e persistência de dados. As pastas `AP1` e `AP2` contêm as atividades preparatórias avaliativas do semestre.
